...
function set_language_strings_version() {
	...
	language_strings_version="9.30-1"
}
...
function language_strings() {
	...
	arr["FRENCH",620]="${pending_of_translation} La méthode de gestion des fenêtres a été modifiée dans le fichier de configuration. Redémarrez le script pour que les modifications prennent effet"
	arr["CATALAN",620]="${pending_of_translation} S'ha modificat en el fitxer de configuració el mètode de gestió de finestres. Reinicia el script perquè els canvis tinguin efecte"
	arr["PORTUGUESE",620]="${pending_of_translation} O método de gerenciamento de janelas foi modificado no arquivo de configuração. Reinicie o script para que as alterações entrem em vigor"
	arr["RUSSIAN",620]="${pending_of_translation} Метод управления окнами был изменен в файле конфигурации. Перезапустите скрипт, чтобы изменения вступили в силу"

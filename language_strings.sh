...
function language_strings() {
	...
	arr["CATALAN",630]="${pending_of_translation} Introduïu el codi de país de dues lletres (US, ES, FR):"
	arr["PORTUGUESE",630]="${pending_of_translation} Digite o código do país de duas letras (US, ES, FR):"
	arr["RUSSIAN",630]="${pending_of_translation} Введите двухбуквенный код страны (US, ES, FR):"
	...
	arr["CATALAN",631]="${pending_of_translation} Introduïu l’estat o la província (Madrid, New Jersey):"
	arr["PORTUGUESE",631]="${pending_of_translation} Digite o estado ou a província (Madrid, New Jersey):"
	arr["RUSSIAN",631]="${pending_of_translation} Введите штат или провинцию (Madrid, New Jersey):"
	...
	arr["CATALAN",632]="${pending_of_translation} Introduïu el locale (Hong Kong, Dublin):"
	arr["PORTUGUESE",632]="${pending_of_translation} Digite a localidade (Hong Kong, Dublin):"
	arr["RUSSIAN",632]="${pending_of_translation} Введите язык (Hong Kong, Dublin):"
	...
	arr["CATALAN",633]="${pending_of_translation} Introduïu el nom de l'organització (Evil Corp):"
	arr["PORTUGUESE",633]="${pending_of_translation} Digite o nome da organização (Evil Corp):"
	arr["RUSSIAN",633]="${pending_of_translation} Введите название организации (Evil Corp):"
	...
	arr["CATALAN",634]="${pending_of_translation} Introduïu el correu electrònic (tyrellwellick@ecorp.com):"
	arr["PORTUGUESE",634]="${pending_of_translation} Digite o email (tyrellwellick@ecorp.com):"
	arr["RUSSIAN",634]="${pending_of_translation} Введите адрес электронной почты (tyrellwellick@ecorp.com):"
	...
	arr["CATALAN",635]="${pending_of_translation} Introduïu \"common name\" (CN) per cert (ecorp.com):"
	arr["PORTUGUESE",635]="${pending_of_translation} Digite \"common name\" (CN) para cert (ecorp.com):"
	arr["RUSSIAN",635]="${pending_of_translation} Введите \"common name\" (CN) для сертификата (ecorp.com):"
	...
	arr["FRENCH",645]="${pending_of_translation} ${green_color}Voulez-vous utiliser des certificats personnalisés lors de l'attaque? ${blue_color}Si vous répondez non \"n\", airgeddon en utilisera par défaut. si vous répondez oui \"y\", vous devrez fournir leur chemin et connaître le mot de passe avec lequel ils ont été générés ${normal_color}${visual_choice}"
	arr["CATALAN",645]="${pending_of_translation} ${green_color}Vols fer servir certificats personalitzats durant l'atac? ${blue_color}Si respons no \"n\", airgeddon utilitzarà uns per defecte. si respons sí \"y\" llavors hauràs de proporcionar la ruta dels mateixos i conèixer la contrasenya amb la qual van ser generats ${normal_color}${visual_choice}"
	arr["PORTUGUESE",645]="${pending_of_translation} ${green_color}Você quer usar certificados personalizados durante o ataque? ${blue_color}Se você responder não \"n\", o airgeddon irá usar as por padrão. se você responder sim \"y\", então você terá que fornecer o caminho e saber a senha com a qual eles foram gerados ${normal_color}${visual_choice}"
	arr["RUSSIAN",645]="${pending_of_translation} ${green_color}Хотите использовать пользовательские сертификаты во время атаки? ${blue_color}Если вы ответите «нет», airgeddon будет использовать их по умолчанию. если вы ответите да \"y\", то вам нужно будет указать их путь и знать пароль, с помощью которого они были сгенерированы ${normal_color}${visual_choice}"
	...
	arr["FRENCH",646]="${pending_of_translation} ${blue_color}airgeddon a détecté que des certificats personnalisés ont été générés au cours de cette session. ${green_color}Voulez-vous les utiliser lors de l'attaque? ${normal_color}${visual_choice}"
	arr["CATALAN",646]="${pending_of_translation} ${blue_color}airgeddon ha detectat que han estat generats uns certificats personalitzats durant aquesta sessió. ${green_color}Vols utilitzar-los durant l'atac? ${normal_color}${visual_choice}"
	arr["PORTUGUESE",646]="${pending_of_translation} ${blue_color}airgeddon detectou que certificados personalizados foram gerados durante esta sessão. ${green_color}Você quer usá-los durante o ataque? ${normal_color}${visual_choice}"
	arr["RUSSIAN",646]="${pending_of_translation} ${blue_color}airgeddon обнаружил, что пользовательские сертификаты были созданы во время этого сеанса. ${green_color}вы хотите использовать их во время атаки? ${normal_color}${visual_choice}"
	...
	arr["FRENCH",647]="${pending_of_translation} Les certificats par défaut seront utilisés"
	arr["CATALAN",647]="${pending_of_translation} S'utilitzaran els certificats per defecte"
	arr["PORTUGUESE",647]="${pending_of_translation} Os certificados padrão serão usados"
	arr["RUSSIAN",647]="${pending_of_translation} Сертификаты по умолчанию будут использоваться"
	...
	arr["FRENCH",648]="${pending_of_translation} Les certificats précédemment générés avec airgeddon seront utilisés"
	arr["CATALAN",648]="${pending_of_translation} S'utilitzaran els certificats generats anteriorment amb airgeddon"
	arr["PORTUGUESE",648]="${pending_of_translation} Certificados previamente gerados com airgeddon serão usados"
	arr["RUSSIAN",648]="${pending_of_translation} будут использованы ранее сгенерированные сертификаты с airgeddon"
	...
	arr["FRENCH",649]="${pending_of_translation} Validation des certificats..."
	arr["CATALAN",649]="${pending_of_translation} Validant certificats..."
	arr["PORTUGUESE",649]="${pending_of_translation} Validando certificados..."
	arr["RUSSIAN",649]="${pending_of_translation} Проверка сертификатов..."
	...
	arr["FRENCH",650]="${pending_of_translation} Les certificats sont valables. L'attaque peut être lancée"
	arr["CATALAN",650]="${pending_of_translation} Els certificats són vàlids. L'atac es pot realitzar"
	arr["PORTUGUESE",650]="${pending_of_translation} Os certificados são válidos. O ataque pode ser realizado"
	arr["RUSSIAN",650]="${pending_of_translation} Сертификаты действительны. Можно выполнить атаку"
	...
	arr["FRENCH",651]="${pending_of_translation} Il y a un problème avec les fichiers ou le chemin entré. assurez-vous que les fichiers ca.pem, server.pem et server.key existent dans le chemin indiqué. l'attaque ne peut pas être effectuée"
	arr["CATALAN",651]="${pending_of_translation} Hi ha un problema amb els fitxers o la ruta introduïda. assegura't que els fitxers ca.pem, server.pem i server.key ha a la ruta indicada. l'atac no es pot realitzar"
	arr["PORTUGUESE",651]="${pending_of_translation} Existe um problema com os arquivos ou o caminho digitado. certifique-se de que os arquivos ca.pem, server.pem e server.key existam no caminho indicado. o ataque não pode ser realizado"
	arr["RUSSIAN",651]="${pending_of_translation} возникла проблема с файлами или указанным путем. убедитесь, что файлы ca.pem, server.pem и server.key существуют по указанному пути. атака не может быть выполнена"
	...
	arr["FRENCH",652]="${pending_of_translation} les certificats ne sont pas valides ou le mot de passe est incorrect, donc l'attaque ne peut pas être effectuée"
	arr["CATALAN",652]="${pending_of_translation} Els certificats no són vàlids o la contrasenya és errònia, per la qual cosa l'atac no es pot realitzar"
	arr["PORTUGUESE",652]="${pending_of_translation} Os certificados são inválidos ou a senha está errada, então o ataque não pode ser realizado"
	arr["RUSSIAN",652]="${pending_of_translation} Сертификаты недействительны или неверный пароль, поэтому атака не может быть выполнена"
	...
	arr["FRENCH",653]="${pending_of_translation} Entrez le chemin du répertoire dans lequel se trouvent les fichiers de certificat. ${blue_color}Assurez-vous que les fichiers ca.pem, server.pem et server.key existent dans le chemin indiqué:"
	arr["CATALAN",653]="${pending_of_translation} Introdueix la ruta del directori on es troben els fitxers dels certificats. ${blue_color}Assegura't que els fitxers ca.pem, server.pem i server.key ha a la ruta indicada:"
	arr["PORTUGUESE",653]="${pending_of_translation} Digite o caminho do diretório onde os arquivos de certificado estão localizados. ${blue_color}Certifique-se de que os arquivos ca.pem, server.pem e server.key existam no caminho indicado:"
	arr["RUSSIAN",653]="${pending_of_translation} введите путь к каталогу, в котором находятся файлы сертификатов. ${blue_color}Убедитесь, что файлы ca.pem, server.pem и server.key существуют по указанному пути:"
	...
	arr["FRENCH",654]="${pending_of_translation} Entrez le mot de passe utilisé lors de la création des fichiers de certificat. ${blue_color}N'oubliez pas que s'ils ont été générés avec airgeddon, le mot de passe utilisé était \"airgeddon\":"
	arr["CATALAN",654]="${pending_of_translation} Introdueix la contrasenya utilitzada en la creació dels fitxers dels certificats. ${blue_color}Recorda que si van ser generats amb airgeddon, la contrasenya que es va utilitzar va ser \"airgeddon\":"
	arr["PORTUGUESE",654]="${pending_of_translation} Digite a senha usada na criação dos arquivos de certificado. ${blue_color}Lembre-se de que, se eles foram gerados com airgeddon, a senha usada foi \"airgeddon\":"
	arr["RUSSIAN",654]="${pending_of_translation} введите пароль, использованный при создании файлов сертификата. ${blue_color}Помните, что если они были созданы с помощью airgeddon, то использовался пароль \"airgeddon\":"

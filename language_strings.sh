...
function language_strings() {
	...
	arr["PORTUGUESE",326]="${pending_of_translation} Os certificados são inválidos ou expiraram, portanto, o ataque não pode ser executado"
	...
	arr["ITALIAN",326]="${pending_of_translation} I certificati non sono validi o scaduti, quindi l'attacco non può essere eseguito"
	...
	arr["GREEK",329]="${pending_of_translation} Καταχωρίστε τον κωδικό πρόσβασης που χρησιμοποιείται στη δημιουργία των αρχείων πιστοποιητικών. ${blue_color}Να θυμάστε ότι αν δημιουργήθηκαν με airgeddon, ο κωδικός που χρησιμοποιήθηκε ήταν ${normal_color}\"airgeddon\"${blue_color}:"
	arr["ITALIAN",329]="${pending_of_translation} Immettere la password utilizzata nella creazione dei file del certificato. ${blue_color}Ricorda che se sono stati generati con airgeddon, la password che è stata utilizzata era ${normal_color}\"airgeddon\"${blue_color}:"
	...
	arr["PORTUGUESE",330]="${pending_of_translation} A chave privada não é válida ou a senha está errada, então o ataque não pode ser executado"
	...
	arr["ITALIAN",330]="${pending_of_translation} La chiave privata non è valida o la password è errata, quindi l'attacco non può essere eseguito"
	...
	arr["ENGLISH",652]="For enterprise attacks, if you already have the legitimate certificates of the target network and the password, you can use them. If you don't have them, you can use airgeddon to generate some certificates with the same data and use them to make the attack more credible"
	arr["SPANISH",652]="Para los ataques enterprise, si tienes los certificados legítimos de la red objetivo y su contraseña, podrás utilizarlos. Si no los tienes, puedes generar con airgeddon unos certificados con los mismos datos y utilizarlos para que el ataque sea más creíble"
	arr["FRENCH",652]="${pending_of_translation} Pour les attaques d'entreprise, si vous disposez des certificats légitimes du réseau cible et de votre mot de passe, vous pouvez les utiliser. Si vous ne les avez pas, vous pouvez générer avec airgeddon des certificats avec les mêmes données et les utiliser pour rendre l'attaque plus crédible"
	arr["CATALAN",652]="${pending_of_translation} Per als atacs enterprise, si tens els certificats legítims de la xarxa objectiu i la contrasenya, podràs utilitzar-los. Si no els tens, pots generar amb airgeddon uns certificats amb les mateixes dades i utilitzar-los per que l'atac sigui més creïble"
	arr["PORTUGUESE",652]="${pending_of_translation} Para ataques corporativos, se você tiver os certificados legítimos da rede de destino e sua senha, poderá usá-los. Se você não tiver, você pode gerar com certificados de aireddon com os mesmos dados e usá-los para tornar o ataque mais confiável"
	arr["RUSSIAN",652]="${pending_of_translation} Для корпоративных атак, если у вас есть действительные сертификаты целевой сети и ваш пароль, вы можете использовать их. Если у вас их нет, вы можете создать с помощью сертификатов Airgeddon те же данные и использовать их для повышения вероятности атаки"
	arr["GREEK",652]="${pending_of_translation} Για επιχειρηματικές επιθέσεις, εάν έχετε τα νόμιμα πιστοποιητικά του δικτύου προορισμού και τον κωδικό πρόσβασής σας, μπορείτε να τα χρησιμοποιήσετε. Αν δεν τα έχετε, μπορείτε να δημιουργήσετε με πιστοποιητικά airgeddon τα ίδια δεδομένα και να τα χρησιμοποιήσετε για να κάνετε την επίθεση πιο αξιόπιστη"
	arr["ITALIAN",652]="${pending_of_translation} Per gli attacchi aziendali, se si dispone dei certificati legittimi della rete di destinazione e della password, è possibile utilizzarli. Se non li hai, puoi generare con i certificati airgeddon con gli stessi dati e usarli per rendere l'attacco più credibile"
	arr["POLISH",652]="${pending_of_translation} W przypadku ataków korporacyjnych, jeśli masz prawidłowe certyfikaty sieci docelowej i hasło, możesz ich użyć. Jeśli ich nie masz, możesz wygenerować za pomocą certyfikatów airgeddon z tymi samymi danymi i wykorzystać je, aby atak był bardziej wiarygodny"
	arr["GERMAN",652]="${pending_of_translation} Wenn Sie bei Unternehmensangriffen über die legitimen Zertifikate des Zielnetzwerks und Ihr Kennwort verfügen, können Sie diese verwenden. Wenn Sie sie nicht haben, können Sie mit Airgeddon-Zertifikaten mit denselben Daten erstellen und sie verwenden, um den Angriff glaubwürdiger zu machen"
	arr["TURKISH",652]="${pending_of_translation} Kurumsal saldırılarda, hedef ağın yasal sertifikalarına ve şifreniz varsa bunları kullanabilirsiniz. Onlara sahip değilseniz, aynı verilere sahip airgeddon sertifikalarıyla oluşturabilir ve saldırıyı daha güvenilir hale getirmek için bunları kullanabilirsiniz"

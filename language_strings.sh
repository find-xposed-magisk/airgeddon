...
function language_strings() {
	...
	arr["ENGLISH",636]="There is an error in your configuration. Please check the ${rc_file} file. You have enabled the option to force the use of mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) but your system doesn't have mdk3 installed. Disable this option or install mdk3 to fix the problem"
	arr["SPANISH",636]="Hay un error en tu configuración. Por favor revisa el fichero ${rc_file}. Has activado la opción para forzar el uso de mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) pero tu sistema no tiene mdk3 instaladas. Deshabilita esta opción o instala mdk3 para solucionar el problema"
	arr["FRENCH",636]="${pending_of_translation} Il y a une erreur dans votre configuration. Veuillez vérifier le fichier ${rc_file}. Vous avez activé l'option pour forcer l'utilisation de mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) mais mdk3 n'est pas installé sur votre système. Désactivez cette option ou installez mdk3 pour résoudre le problème"
	arr["CATALAN",636]="${pending_of_translation} Hi ha un error en la configuració. Si us plau comprova el fitxer ${rc_file}. Has activat l'opció per forçar l'ús d'mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) però el teu sistema no té mdk3 instal·lades. Deshabilita aquesta opció o instal·la mdk3 per solucionar el problema"
	arr["PORTUGUESE",636]="${pending_of_translation} Existe um erro na sua configuração. Por favor, verifique o arquivo ${rc_file}. Você ativou a opção para forçar o uso de mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}), mas seu sistema não possui o mdk3 instalado. Desabilite esta opção ou instale o mdk3 para resolver o problema"
	arr["RUSSIAN",636]="${pending_of_translation} В вашей конфигурации имеется ошибка. Пожалуйста, проверьте файл ${rc_file}. Вы включили опцию принудительного использования mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}), но в вашей системе не установлены mdk3. Отключите эту опцию или установите mdk3, чтобы исправить проблему"
	arr["GREEK",636]="${pending_of_translation} Υπάρχει ένα σφάλμα στο configuration σας. Ελέγξτε το αρχείο ${rc_file}. Έχετε ενεργοποιήσει την επιλογή που εξαναγκάζει τη χρήση του mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) και το σύστημά σας δεν έχει το έχει εγκατεστημένο (mdk3). Απενεργοποιήστε την επιλογή αυτή ή εγκαταστήστε το mdk3 για να επιλυθεί το πρόβλημα"
	arr["ITALIAN",636]="${pending_of_translation} C'è un errore nella tua configurazione. Per favore controlla il file ${rc_file}. Hai attivato l'opzione per forzare l'uso di mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) ma il tuo sistema non ha mdk3 installato. Disabilita questa opzione o installa mdk3 per risolvere il problema"
	arr["POLISH",636]="${pending_of_translation} Wystąpił błąd konfiguracji. Sprawdź plik ${rc_file}. Aktywowałeś opcję wymuszania użycia mdk3 (${normal_color}AIRGEDDON_FORCE_MDK${red_color}), ale twój system nie ma zainstalowanego mdk3. Wyłącz tę opcję lub zainstaluj mdk3, aby rozwiązać problem"
	arr["GERMAN",636]="${pending_of_translation} Ein Fehler wurde in Ihren Einstellungen gefunden. Bitte überprüfen Sie die Datei ${rc_file}. Sie haben die Option aktiviert, um die Verwendung von mdk3 zu erzwingen (${normal_color}AIRGEDDON_FORCE_MDK${red_color}), aber auf Ihrem System sind mdk3 nicht installiert. Deaktivieren Sie diese Option oder installieren Sie mdk3, um das Problem zu lösen"
	arr["TURKISH",636]="${pending_of_translation} Yapılandırmanızda bir hata var. Lütfen ${rc_file} dosyasını kontrol edin. (${normal_color}AIRGEDDON_FORCE_MDK${red_color}) mdk3 kullanımını zorlama seçeneğini etkinleştirdiniz, ancak sisteminizde mdk3 kurulu değil. Bu seçeneği devre dışı bırakın veya sorunu çözmek için mdk3 yükleyin"

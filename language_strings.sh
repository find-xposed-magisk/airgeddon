...
function initialize_language_strings() {
	...
	arr["FRENCH",684]="\${pending_of_translation} \${blue_color}Le fichier PMKID que vous avez capturé est au format texte avec le hachage afin qu'il puisse être craqué à l'aide de hashcat. De plus, airgeddon peut transformer la capture au format \".cap\" afin que le fichier puisse être cracké avec aircrack-ng comme s'il s'agissait d'une capture airodump-ng, mais il nécessitera la commande tshark pour effectuer cette transformation. \${green_color}Voulez-vous effectuer la transformation? \${pink_color}(vous conserverez également le fichier hashcat, ceci est supplémentaire) \${normal_color}\${visual_choice}"
	arr["CATALAN",684]="\${pending_of_translation} \${blue_color}El fitxer PMKID que has capturat està en un format de text amb el hash perquè es pugui crackejar usant hashcat. Addicionalment, airgeddon pot transformar la captura a format \".cap\" perquè el fitxer pugui ser esquerdat amb aircrack-ng com si es tractés d'una captura de airodump-ng, però requerirà d'la comanda tshark per realitzar aquesta transformació. \${green_color}¿Vols fer la transformació? \${pink_color}(mantindràs també el fitxer de hashcat, això és addicional) \${normal_color}\${visual_choice}"
	arr["PORTUGUESE",684]="\${pending_of_translation} \${blue_color}O arquivo PMKID que você capturou está em um formato de texto com o hash, para que possa ser quebrado usando o hashcat. Além disso, o airgeddon pode transformar a captura no formato \".cap\" para que o arquivo possa ser quebrado com o aircrack-ng como se fosse uma captura do airodump-ng, mas será necessário o comando tshark para executar essa transformação. \${green_color}Deseja executar a transformação? \${pink_color}(você também manterá o arquivo hashcat, isso é adicional) \${normal_color}\${visual_choice}"
	...
	arr["GREEK",684]="\${pending_of_translation} \${blue_color}Το ληφθέν αρχείο PMKID είναι σε μορφή κειμένου που περιέχει τον κατακερματισμό, προκειμένου να καταστραφεί χρησιμοποιώντας το hashcat. Επιπλέον, το airgeddon μπορεί να μετατρέψει τη λήψη σε μορφή \".cap\" για να αφήσει το αρχείο να σπάσει χρησιμοποιώντας aircrack-ng σαν να ήταν σύλληψη airodump-ng, αλλά απαιτείται εντολή tshark για να είναι σε θέση να πραγματοποιήσει αυτόν τον μετασχηματισμό. \${green_color}Θέλετε να πραγματοποιήσετε τη μετατροπή; \${pink_color}(θα διατηρήσετε επίσης το αρχείο hashcat, αυτό είναι επιπλέον) \${normal_color}\${visual_choice}"
	arr["ITALIAN",684]="\${pending_of_translation} \${blue_color}Il file PMKID che hai acquisito è in un formato di testo con l'hash in modo che possa essere crackato usando hashcat. Inoltre, airgeddon può trasformare la cattura in formato \".cap\" in modo che il file possa essere crackato con aircrack-ng come se fosse una cattura airodump-ng, ma richiederà il comando tshark per eseguire questa trasformazione. \${green_color}Vuoi eseguire la trasformazione? \${pink_color}(manterrai anche il file hashcat, questo è aggiuntivo) \${normal_color}\${visual_choice}"
	...
	arr["FRENCH",685]="\${pending_of_translation} Erreur. Il semble que vous ne disposiez pas de la commande tshark. Selon le système, le paquet qui le contient peut être tshark, wireshark-cli, wireshark, etc. Installez-le avant d'essayer cette transformation la prochaine fois. La transformation ne peut pas être effectuée"
	arr["CATALAN",685]="\${pending_of_translation} Error. Sembla que no tens disponible la comanda tshark. Depenent de el sistema, el paquet que el conté pot ser tshark, wireshark-cli, wireshark, etc. Instal·la-abans d'intentar aquesta transformació la propera vegada. La transformació no es pot dur a terme"
	arr["PORTUGUESE",685]="\${pending_of_translation} Erro. Parece que você não tem o comando tshark disponível. Dependendo do sistema, o pacote que o contém pode ser tshark, wireshark-cli, wireshark etc. Instale-o antes de tentar esta transformação na próxima vez. A transformação não pode ser realizada"
	...
	arr["TURKISH",685]="\${pending_of_translation} Hata. Görünüşe göre tshark komutu yok. Sisteme bağlı olarak, onu içeren paket tshark, wireshark-cli, wireshark vb. Olabilir. Bir dahaki sefere bu dönüşümü denemeden önce kurun. Dönüşüm yapılamaz"

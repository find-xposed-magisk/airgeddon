...
function language_strings() {
	...
	arr["PORTUGUESE",119]="5.  Menu ferramentas Handshake/PMKID"
	arr["RUSSIAN",119]="${pending_of_translation} 5.  Меню инструментов для работы с рукопожатием/PMKID"
	arr["GREEK",119]="${pending_of_translation} 5.  Μενού εργαλειών Χειραψίας/PMKID"
	...
	arr["RUSSIAN",120]="${pending_of_translation} Меню инструментов для работы с рукопожатием/PMKID"
	arr["GREEK",120]="${pending_of_translation} Μενού εργαλειών Χειραψίας/PMKID"
	...
	arr["PORTUGUESE",127]="A ordem normal para esse menu é: 1-Escolha de uma interface wifi 2-colocar interface wifi no modo monitor 3-Selecionar uma rede 4-Capturar Handshake/PMKID"
	arr["RUSSIAN",127]="${pending_of_translation} Естественный порядок работы в этом меню: 1-Выбрать wifi карту 2-Перевести её в режим монитора 3-Выбрать целевую сеть 4-Захватить рукопожатие/PMKID"
	arr["GREEK",127]="${pending_of_translation} Η σειρά εντολών για να προχωρήσετε σε αυτό το μενού είναι συνήθως: 1-Επιλέξτε κάρτα wifi 2-Βάλτε την σε κατάσταση παρακολούθησης 3-Επιλέξτε δίκτυο-στόχος 4-Καταγράψτε την Χειραψία/PMKID"
	...
	arr["PORTUGUESE",136]="${pending_of_translation} Apenas algumas redes são vulneráveis a capturar um pacote contendo um PMKID. Para fazer isso, eles devem ter ativado recursos como 802.11r de roaming"
	...
	arr["ITALIAN",136]="${pending_of_translation} Solo alcune reti sono vulnerabili ad acquisire un pacchetto contenente un PMKID. Per fare questo devono aver attivato le caratteristiche come 802.11r di roaming"
	...
	arr["RUSSIAN",172]="${pending_of_translation} 1.  (aircrack) Атака по словарю в отношении захваченного Handshake/PMKID файла"
	arr["GREEK",172]="${pending_of_translation} 1.  (aircrack) Επίθεση με χρήση λεξικού σε αρχείο καταγραφής Handshake/PMKID"
	...
	arr["RUSSIAN",175]="${pending_of_translation} 2.  (aircrack + crunch) Атака методом грубой силы в отношении захваченного Handshake/PMKID файла"
	...
	arr["RUSSIAN",230]="${pending_of_translation} 3.  (hashcat) Атака по словарю в отношению захваченного Handshake файла"
	arr["GREEK",230]="${pending_of_translation} 3.  (hashcat) Επίθεση με χρήση λεξικού σε αρχείο καταγραφής Handshake"
	...
	arr["GERMAN",230]="${pending_of_translation} 3.  (hashcat) Wörterbuchangriff auf die Aufnahmedatei Handshake"
	arr["TURKISH",230]="${pending_of_translation} 3.  (hashcat) Yakalama Handshake dosyasına karşı sözlük saldırısı"
	...
	arr["RUSSIAN",231]="${pending_of_translation} 4.  (hashcat) Атака рукопожатия полным перебором Handshake"
	arr["GREEK",231]="${pending_of_translation} 4.  (hashcat) Επίθεση ωμής βίας σε αρχείο καταγραφής Handshake"
	...
	arr["GERMAN",231]="${pending_of_translation} 4.  (hashcat) Brute-Force-Angriff auf die Capture-Handshake-Datei"
	arr["TURKISH",231]="${pending_of_translation} 4.  (hashcat) Yakalama Handshake dosyasına karşı bruteforce saldırısı"
	...
	arr["RUSSIAN",232]="${pending_of_translation} 5.  (hashcat) Атака на основе правила в отношении захваченного Handshake файла"
	arr["GREEK",232]="${pending_of_translation} 5.  (hashcat) Επίθεση κανόνων σε αρχείο καταγραφής Handshake"
	...
	arr["GERMAN",232]="${pending_of_translation} 5.  (hashcat) Regeln-basierender Angriff gegen der Capture-Handshake-Datei"
	arr["TURKISH",232]="${pending_of_translation} 5.  (hashcat) Yakalama Handshake dosyasına karşı kural tabanlı saldırı"
	...
	arr["RUSSIAN",663]="${pending_of_translation} 5.  Захват PMKID"
	arr["GREEK",663]="${pending_of_translation} 5.  Καταγράψτε την PMKID"
	...
	arr["CATALAN",665]="${pending_of_translation} A les xarxes WPA/WPA2-PSK, pot esquerdar ja sigui PMKIDs o Handshakes per obtenir la contrasenya de xarxa"
	arr["PORTUGUESE",665]="${pending_of_translation} Em redes WPA/WPA2-PSK que você pode rachar qualquer PMKIDs ou Handshakes para obter a senha da rede"
	arr["RUSSIAN",665]="${pending_of_translation} В WPA/WPA2-PSK сети вы можете взломать либо PMKIDs или Handshakes, чтобы получить сетевую кодовую фразу"
	...
	arr["ITALIAN",665]="${pending_of_translation} Nelle reti WPA/WPA2-PSK, si possono rompere sia PMKIDs o Handshakes per ottenere la passphrase di rete"
	...
	arr["GERMAN",665]="${pending_of_translation} In WPA/WPA2-PSK Netzwerken können Sie entweder PMKIDs oder Handshakes knacken das Netzwerk Passwort zu erhalten"
	arr["TURKISH",665]="${pending_of_translation} WPA/WPA2-PSK ağlarda ağ parolasını elde etmek ya PMKIDs veya Handshakes çatlaklara yol açabilir"
	...
	arr["PORTUGUESE",666]="${pending_of_translation} Se a captura de um aperto de mão ou um PMKID, você pode tentar decifrar o menu descriptografia WPA/WPA2 desligada, escolhendo o submenu tipo \"Pessoal\""
	...
	arr["GREEK",666]="${pending_of_translation} Αν έχετε συλλάβει μια χειραψία ή ένα PMKID, μπορείτε να προσπαθήσετε να αποκρυπτογραφήσει το μενού αποκρυπτογράφηση WPA/WPA2 εκτός από την επιλογή του υπομενού τύπου \"Personal\""
	arr["ITALIAN",666]="${pending_of_translation} Se l'acquisizione di una stretta di mano o un PMKID, si può provare a decifrare il menu di decifratura WPA/WPA2 offline scegliendo il sottomenu tipo \"Personal\""
	...
	arr["GERMAN",666]="${pending_of_translation} Wenn ein Händedruck oder ein PMKID aufnehmen, können Sie versuchen, das Menü zu entschlüsseln Entschlüsselung WPA/WPA2 offline durch die Art Untermenü \"Persönlich\" die Wahl"
	arr["TURKISH",666]="${pending_of_translation} Bir el sıkışma veya PMKID yakalama varsa, menü deşifre deneyebilirsiniz şifre çözme WPA/WPA2 çevrimdışı tip alt menüsünü \"Personal\" seçerek"
	...
	arr["PORTUGUESE",667]="${pending_of_translation} Uma versão antiga do aircrack foi detectada. Não há problema se você planeja descriptografar um Handshake, mas é necessário levar em consideração para descriptografar o PMKID, pelo menos a versão ${aircrack_pmkid_version} do aircrack é necessária. Caso contrário o PMKID não será detectado"
	...
	arr["GREEK",667]="${pending_of_translation} Έχει εντοπιστεί ότι έχετε εγκαταστήσει μια παλιά έκδοση aircrack. Δεν υπάρχει πρόβλημα αν σκοπεύετε να σπάσει μια χειραψία, αλλά έχουμε κατά νου ότι για να σπάσει PMKID χρειάζεται ${aircrack_pmkid_version} ως ελάχιστη έκδοση. Διαφορετικά PMKID δεν θα πρέπει να ανιχνεύεται"
	arr["ITALIAN",667]="${pending_of_translation} È stata rilevata una vecchia versione di aircrack. Nessun problema se prevedi di decrittografare un Handshake, ma devi prendere in considerazione per decrittografare PMKID, è richiesta almeno la versione ${aircrack_pmkid_version} di aircrack. Altrimenti il PMKID non verrà rilevato"
	...
	arr["GERMAN",667]="${pending_of_translation} Es wurde festgestellt, dass Sie eine alte aircrack Version installiert haben. Kein Problem, wenn Sie planen, einen Händedruck zu knacken, aber bedenken Sie, dass PMKID zu knacken ist ${aircrack_pmkid_version} als Mindestversion benötigt. Sonst wird PMKID nicht erkannt werden"
	arr["TURKISH",667]="${pending_of_translation} Eski bir aircrack sürümünün yüklü olduğunu tespit edildi. PMKID çatlak akılda hiçbir Bir Handshake çatlak planlıyorsanız sorun, ama ayı asgari versiyonu olarak ${aircrack_pmkid_version} gereklidir. Aksi takdirde PMKID algılanmaz"
	...
	arr["RUSSIAN",668]="${pending_of_translation} 6.  (hashcat) Атака по словарю в отношению захваченного PMKID файла"
	arr["GREEK",668]="${pending_of_translation} 6.  (hashcat) Επίθεση με χρήση λεξικού σε αρχείο καταγραφής PMKID"
	...
	arr["GERMAN",668]="${pending_of_translation} 6.  (hashcat) Wörterbuchangriff auf die Aufnahmedatei PMKID"
	arr["TURKISH",668]="${pending_of_translation} 6.  (hashcat) Yakalama PMKID dosyasına karşı sözlük saldırısı"
	...
	arr["RUSSIAN",669]="${pending_of_translation} 7.  (hashcat) Атака рукопожатия полным перебором PMKID"
	arr["GREEK",669]="${pending_of_translation} 7.  (hashcat) Επίθεση ωμής βίας σε αρχείο καταγραφής PMKID"
	...
	arr["GERMAN",669]="${pending_of_translation} 7.  (hashcat) Brute-Force-Angriff auf die Capture-PMKID-Datei"
	arr["TURKISH",669]="${pending_of_translation} 7.  (hashcat) Yakalama PMKID dosyasına karşı bruteforce saldırısı"
	...
	arr["RUSSIAN",670]="${pending_of_translation} 8.  (hashcat) Атака на основе правила в отношении захваченного PMKID файла"
	arr["GREEK",670]="${pending_of_translation} 8.  (hashcat) Επίθεση κανόνων σε αρχείο καταγραφής PMKID"
	...
	arr["GERMAN",670]="${pending_of_translation} 8.  (hashcat) Regeln-basierender Angriff gegen der Capture-PMKID-Datei"
	arr["TURKISH",670]="${pending_of_translation} 8.  (hashcat) Yakalama PMKID dosyasına karşı kural tabanlı saldırı"
	...
	arr["PORTUGUESE",671]="${pending_of_translation} Não feche la janela manualmente, o script fechará quando necessário. Em cerca de ${timeout} segundos no máximo você vai saber se tem o PMKID"
	...
	arr["GREEK",671]="${pending_of_translation} Μην κλείνετε χειροκίνητα το παράθυρο, εάν χρειαστεί το script θα το κάνει μόνο του. Σε περίπου ${timeout} δευτερόλεπτα το μέγιστο θα μάθετε αν αποκτήσατε την PMKID"
	arr["ITALIAN",671]="${pending_of_translation} Non chiudere la finestra manualmente, lo script lo fará cuando necessario. In ${timeout} secondi al massimo scoprirai se hai catturato l'PMKID"
	...
	arr["GERMAN",671]="${pending_of_translation} Schließen Sie das Fenster nicht manuell, das Skript wird dies bei Bedarf tun. In ungefähr ${timeout} Sekunden werden Sie wissen, ob Sie den PMKID bekommen haben"
	arr["TURKISH",671]="${pending_of_translation} Pencereyi manuel olarak kapatmayın, yazılım gerektiğinde yapacaktır. Yaklaşık ${timeout} saniye içinde, PMKID'e sahip olup olmadığınızı bileceksiniz"
	...
	arr["PORTUGUESE",672]="${pending_of_translation} Parece que nos falhamos... ente novamente, escolha outro objetivo ou aumente o timeout. O ponto de acesso pode não estar vulnerável"
	...
	arr["GREEK",672]="${pending_of_translation} Φαίνεται πως αποτύχαμε... δοκιμάστε ξανά, επιλέξτε άλλο στόχο ή αυξήστε το timeout. Ίσως το σημείο πρόσβασης να μην είναι ευάλωτο"
	arr["ITALIAN",672]="${pending_of_translation} Sembra che abbiamo fallito... riprova, scegli un altro obiettivo o aumenta il timeout. Il punto di accesso potrebbe non essere vulnerabile"
	...
	arr["GERMAN",672]="${pending_of_translation} Anscheinend haben wir es nicht geschafft... Versuchen Sie es erneut, wählen Sie ein anderes Ziel oder erhöhen Sie das Timeout. Der Zugangspunkt ist möglicherweise nicht anfällig"
	arr["TURKISH",672]="${pending_of_translation} Başarısız görünüyoruz... tekrar deneyin, başka bir hedef seçin veya zaman aşımını artırın. Erişim noktası savunmasız olmayabilir"
	...
	arr["PORTUGUESE",673]="${pending_of_translation} Arquivo PMKID gerado com sucesso [${normal_color}${enteredpath}${blue_color}]"
	arr["RUSSIAN",673]="${pending_of_translation} Файл PMKID успешно создан в [${normal_color}${enteredpath}${blue_color}]"
	arr["GREEK",673]="${pending_of_translation} Το αρχείο PMKID δημιουργήθηκε επιτυχώς στο [${normal_color}${enteredpath}${blue_color}]"
	arr["ITALIAN",673]="${pending_of_translation} PMKID del file generato con successo in [${normal_color}${enteredpath}${blue_color}]"
	arr["POLISH",673]="${pending_of_translation} Plik PMKID został pomyślnie wygenerowany w [${normal_color}${enteredpath}${blue_color}]"
	arr["GERMAN",673]="${pending_of_translation} PMKID-Datei erfolgreich in [${normal_color}${enteredpath}${blue_color}] erstellt"
	arr["TURKISH",673]="${pending_of_translation} PMKID dosyası şu hedefte başarıyla oluşturuldu [${normal_color}${enteredpath}${blue_color}]"
	...
	arr["GREEK",678]="${pending_of_translation} Έχετε εγκατεστημένο το hashcat (v${hashcat_version}). Έχετε την απαραίτητη έκδοση hashcat (ελάχιστη έκδοση v${minimum_hashcat_pmkid_version}), για την επίθεση PMKID με ενσωματωμένο hashcat. Το script μπορεί να συνεχίσει..."

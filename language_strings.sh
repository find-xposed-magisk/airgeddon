...
function language_strings() {
	...
	arr["PORTUGUESE",301]="${pending_of_translation} Para ataques corporativos, se você tiver os certificados legítimos da rede de destino e sua senha, poderá usá-los. Se você não tiver, você pode gerar com certificados de aireddon com os mesmos dados e usá-los para tornar o ataque mais confiável"
	...
	arr["CATALAN",590]="${pending_of_translation} Pots modificar manualment el fitxer ${rc_path} per modificar les seves opcions. També pots llançar \"flags\" a la línia d'ordres. Més informació a: ${urlgithub_wiki}/Options"
	arr["PORTUGUESE",590]="${pending_of_translation} Você pode modificar manualmente o arquivo ${rc_path} para modificar suas opções. Você também pode usar argumentos ao executar airgeddon na linha de comando. Mais informações sobre: ${urlgithub_wiki}/Options"
	...
	arr["GREEK",626]="${pending_of_translation} Από την εκδοχή airgeddon 9.20, υποστηρίζεται το tmux και μπορεί να χρησιμοποιηθεί αντί του xterm ως χειριστής παραθύρων. Το σενάριο μπορεί να εκτοξευθεί σε περιβάλλον χωρίς κεφαλή χωρίς γραφικό σύστημα παραθύρων X. Συνιστάται μόνο για προχωρημένους χρήστες. Όπως και κάθε άλλη επιλογή, μπορεί να ρυθμιστεί στο μενού επιλογών, στο αρχείο επιλογών ${rc_path} ή να ξεκινήσει χρησιμοποιώντας τη \"flag\" AIRGEDDON_WINDOWS_HANDLING στη γραμμή εντολών. Περισσότερες πληροφορίες σχετικά με την προσαρμογή των επιλογών στο Wiki: ${urlgithub_wiki}/Options"
	...
	arr["GREEK",636]="${pending_of_translation} Υπάρχει ένα σφάλμα στο configuration σας. Ελέγξτε το αρχείο ${rc_path}. Έχετε ενεργοποιήσει την επιλογή που εξαναγκάζει τη χρήση του mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}) και το σύστημά σας δεν έχει το έχει εγκατεστημένο (mdk3). Απενεργοποιήστε την επιλογή αυτή ή εγκαταστήστε το mdk3 για να επιλυθεί το πρόβλημα"
	arr["ITALIAN",636]="${pending_of_translation} C'è un errore nella tua configurazione. Per favore controlla il file ${rc_path}. Hai attivato l'opzione per forzare l'uso di mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}) ma il tuo sistema non ha mdk3 installato. Disabilita questa opzione o installa mdk3 per risolvere il problema"
